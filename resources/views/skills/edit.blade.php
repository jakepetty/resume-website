...
<section class="container">
    <form action="{{ route('skills.update', $skill->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control {{ $errors->has('level') ? 'is-invalid' : ($errors->any() ? 'is-valid' : null) }}" id="skill-level" name="level" min="0" max="100" value="{{ old('level', $skill->level) }}" value="{{ old('level', $skill->level) }}" required>
            @if($errors->has('level'))
